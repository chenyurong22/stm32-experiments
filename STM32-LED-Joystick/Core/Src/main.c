...
void Set_LED(int led_index, int Red, int Green, int Blue) {
	led_data[led_index][0] = Green;
	led_data[led_index][1] = Red;
...
void WS2812_Send(void) {
	...
	for (int i = 0; i < MAX_LED; i++) {
		color = ((led_data[i][0] << 16) | (led_data[i][1] << 8)
				| (led_data[i][2]));
	...
	HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_1, (uint32_t*) pwm_data, indx);
}
